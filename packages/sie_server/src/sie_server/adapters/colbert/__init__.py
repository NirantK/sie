...
class ColBERTAdapter(ModelAdapter):
    ...
    def _encode_native(
        ...
        # For query expansion, attend to ALL tokens including MASK expansion tokens
        if use_expansion:
        ...
        with torch.inference_mode():
            ...
            for i in range(len(texts)):
                # Get non-padded tokens (for expansion, all tokens are "attended")
                mask = attention_mask[i].bool()
                seq_hidden = hidden[i][mask]
                seq_ids = input_ids[i][mask]

                # Filter special tokens if configured
                # Note: For query expansion, MASK tokens are NOT filtered (they're semantic)
                if self._skip_special_tokens:
                    seq_hidden = self._filter_special_tokens(seq_hidden, seq_ids, keep_mask=use_expansion)
    ...
    def _filter_special_tokens(
        ...
        *,
        ...
        """Filter out special tokens from hidden states.
    ...
    def score(
        ...
        *,
        ...
        for doc_vecs in doc_output.multivector:
            ...
            # sim shape: [num_query_tokens, num_doc_tokens]
    ...
    def _split_embeddings(
        ...
        *,
        ...
        Optionally filters out special tokens ([CLS], [SEP], [PAD]).
        ...
        for i in range(num_seqs):
            ...
            seq_hidden = hidden[start:end]
            seq_ids = input_ids[start:end]

            if self._skip_special_tokens and special_ids:
                # Filter out special tokens
                mask = torch.tensor(
                    [tok_id.item() not in special_ids for tok_id in seq_ids],
                    device=self._device,
                )
                seq_hidden = seq_hidden[mask]

            results.append(seq_hidden.float().cpu().numpy())
