...
class ColPaliAdapter(ModelAdapter):
    ...
    def _fix_config_dtype(self, config: Any, dtype: torch.dtype) -> None:
        ...
        if hasattr(config, "text_config") and config.text_config is not None:
            config.text_config.torch_dtype = dtype_str

        # Fix vision_config dtype
        if hasattr(config, "vision_config") and config.vision_config is not None:
            if hasattr(config.vision_config, "torch_dtype"):
                config.vision_config.torch_dtype = dtype_str
        ...
        if hasattr(config, "vlm_config") and config.vlm_config is not None:
            ...
            if hasattr(config.vlm_config, "vision_config") and config.vlm_config.vision_config is not None:
                if hasattr(config.vlm_config.vision_config, "torch_dtype"):
                    config.vlm_config.vision_config.torch_dtype = dtype_str
    ...
    def _encode_prepared_batch(
        ...
        *,
        ...
        # to pixel_values. Generate them using dummy images - the input_ids are the same
        # for all images of the same size since they just contain image placeholder tokens.
        batch_size = len(prepared_items)
        dummy_images = [Image.new("RGB", (448, 448), color="white") for _ in range(batch_size)]
        batch_inputs = self._processor(images=dummy_images, return_tensors="pt")
