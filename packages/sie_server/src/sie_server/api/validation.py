"""Request validation helpers for SIE Server API.

Common validation functions used across encode, score, and extract endpoints.
Provides lightweight validation for request bodies without Pydantic overhead.
"""
...
def validate_encode_request(data: Any) -> None:
    """Validate encode request structure.
    ...
    if not isinstance(data, dict):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"code": ErrorCode.INVALID_INPUT.value, "message": "Request body must be a JSON/msgpack object"},
        )
    ...
    if items is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"code": ErrorCode.INVALID_INPUT.value, "message": "Field 'items' is required"},
        )
    if not isinstance(items, list):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"code": ErrorCode.INVALID_INPUT.value, "message": "Field 'items' must be a list"},
        )
    if len(items) == 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"code": ErrorCode.INVALID_INPUT.value, "message": "Field 'items' must not be empty"},
        )
    ...
    for i, item in enumerate(items):
        if not isinstance(item, dict):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"code": ErrorCode.INVALID_INPUT.value, "message": f"Item at index {i} must be an object"},
            )


def validate_score_request(data: Any) -> None:
    """Validate score request structure.
    ...
    if query is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"code": ErrorCode.INVALID_INPUT.value, "message": "Field 'query' is required"},
        )
    if not isinstance(query, dict):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"code": ErrorCode.INVALID_INPUT.value, "message": "Field 'query' must be an object"},
        )
...
def validate_extract_request(data: Any) -> None:
    """Validate extract request structure.
