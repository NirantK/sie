...
class ModelWorker:
    ...
    async def _get_next_batch_fcfs(self) -> tuple[str | None, FormattedBatch[HasCost, RequestMetadata]]:
        ...
        while True:
            ...
            if oldest_lora is not None or (oldest_lora is None and self._batchers[None].pending_count > 0):
                ...
                batch = await self._batchers[selected_lora].get_batch()
                return selected_lora, batch

            # No batchers have pending items - wait briefly and retry
    ...
    def _group_by_inference_config(
        ...
        for prepared_item, metadata in zip(batch.items, batch.metadata, strict=True):
            ...
            items_list, metadata_list, indices_list, prepared_items_list = groups[config_key]
            ...
            prepared_items_list.append(prepared_item)
