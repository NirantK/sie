...
class BatchFormer[I: HasCost, T]:
    ...
    def _should_yield_batch(self) -> bool:
        """Check if batch should be yielded (full or timeout expired)."""
        if len(self._pending) == 0:
            return False
        return self._batch_is_full() or self._batch_timeout_expired()
    ...
    def _get_wait_timeout(self) -> float | None:
        ...
        elapsed_ms = (time.monotonic() - self._first_request_time) * 1000
        remaining_ms = max(0, self._config.max_batch_wait_ms - elapsed_ms)
    ...
    def _extract_batch(self) -> FormattedBatch[I, T]:
        ...
        trigger = "timeout" if was_timeout else ("full" if was_full else "immediate")
        ...
        # Reset timer if we've emptied the queue
