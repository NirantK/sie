...
class TestBatchFormer:
    ...
    @pytest.mark.asyncio
    async def test_get_batch_returns_on_timeout(self, batcher: BatchFormer[TextPreparedItem, str]) -> None:
        """get_batch returns after timeout when batch not full."""
        item = make_text_item([1, 2, 3])
        await batcher.submit(item, "req-1")
        ...
        # Should wait roughly the timeout (1ms)
        assert elapsed >= 0.003  # Allow some timing variance
        assert elapsed < 0.05  # But not too long
        assert batch.size == 1
...
class TestBatchFormerConcurrency:
    ...
    async def test_concurrent_submits(self) -> None:
        ...
        config = BatchConfig(max_batch_requests=10, max_batch_wait_ms=1)
    ...
    async def test_get_batch_waits_for_first_request(self) -> None:
        """get_batch returns after first request plus timeout."""
        # Use a very short timeout so test runs quickly
        config = BatchConfig(max_batch_wait_ms=5, max_batch_requests=10)
        batcher: BatchFormer[TextPreparedItem, str] = BatchFormer(config)

        async def delayed_submit() -> None:
            await asyncio.sleep(0.002)  # Small delay before submit
            item = make_text_item([1, 2, 3])
        ...
        # get_batch will wait for first request, then wait for batch timeout
        start = time.monotonic()
        ...
        # Should have gotten the batch (submit at ~2ms + timeout 5ms = ~7ms)
        assert batch is not None
        assert batch.size == 1
        # Should take at least the batch timeout after first request
        assert elapsed >= 0.005  # At least the batch timeout
