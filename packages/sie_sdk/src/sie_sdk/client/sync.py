...
GPU Selection and Auto-Retry:
    ...
    ...     provision_timeout_s=300,  # Wait up to 5 min
    ... )
...
class SIEClient:
    ...
    def _lease_renewal_loop(self) -> None:
        ...
        while not self._lease_renewal_stop.wait(timeout=DEFAULT_LEASE_RENEWAL_INTERVAL_S):
            try:
                ...
                if response.status_code >= HTTP_CLIENT_ERROR:
                    logger.warning("Failed to renew lease for pool '%s': HTTP %d", pool_name, response.status_code)
                else:
                    logger.debug("Renewed lease for pool '%s'", pool_name)
            except (httpx.HTTPError, OSError) as e:
                logger.warning("Error renewing lease for pool '%s': %s", pool_name, e)
    ...
    def create_pool(self, name: str, gpus: dict[str, int], bundle: str | None = None) -> None:
        ...
        Example:
            >>> client.create_pool("eval", {"l4": 2}, bundle="default")
    ...
    def encode(
        ...
        *,
        ...
        items_for_wire = [convert_item_images({**item}) for item in items_list]  # ty: ignore[invalid-argument-type]
