...
export class SIEClient {
  ...
  // Pool state: track created pools and their lease renewal timers
  private readonly pools: Map<string, { timer: ReturnType<typeof setInterval> }> = new Map();
  ...
  async close(): Promise<void> {
    // Stop all pool lease renewal timers
    for (const [, poolState] of this.pools) {
      clearInterval(poolState.timer);
    }
    this.pools.clear();
  }
  ...
  async createPool(name: string, gpus: Record<string, number>): Promise<void> {
    ...
    try {
      ...
      // Start lease renewal timer for this pool
      const timer = setInterval(async () => {
        try {
          ...
          await fetch(renewUrl, {
            method: "POST",
            headers: renewHeaders,
          });
        } catch {
          // Ignore lease renewal errors - pool will expire if renewal fails
        }
      }, DEFAULT_LEASE_RENEWAL_INTERVAL);

      this.pools.set(name, { timer });
    } catch (error) {
